...
function cargarFichajes() {
    ...
    if (fichajes.length === 0) {
        tbody.innerHTML = `
            <tr class="empty-row">
                <td colspan="7">
                    <p>No hay fichajes disponibles.</p>
                </td>
            </tr>
        `;
    ...
    // Calcular paginación
    const totalPaginas = Math.ceil(fichajes.length / fichajesPorPagina);
    const inicio = (paginaActual - 1) * fichajesPorPagina;
    const fin = inicio + fichajesPorPagina;
    const fichajesPagina = fichajes.slice(inicio, fin);
...
function renderizarPaginacion(container, totalPaginas) {
    ...
    const fin = Math.min(paginaActual * fichajesPorPagina, fichajes.length);
    html += `<div class="paginacion-info">Mostrando ${inicio}-${fin} de ${fichajes.length} fichajes</div>`;
